...
export class PlaybackController {
  async start(req: FastifyRequest, reply: FastifyReply) {
    const { userId, contentId } = req.body as { userId: string; contentId: string }
    try {
  ...
  async update(req: FastifyRequest, reply: FastifyReply) {
    const { playbackId, progress, status } = req.body as { playbackId: string; progress: number; status: 'playing' | 'paused' | 'finished' }
    try {
